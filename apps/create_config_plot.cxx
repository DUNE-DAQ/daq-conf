    ...
    ("help,h", "Provide help message")
      ...
      ("file,f", bpo::value<std::string>(&oksfilename), "OKS database file name")
        ...
        ("root-object,r",
         ...
         "OKS object UID of root vertex; must be session, segment or application")(
          "session,s",
          bpo::value<std::string>(&sessionname)->default_value(""),
  ...
  auto display_help_message = [&options_description]() {
    TLOG() << "create_config_plot : Generate dot graphs from database files" << std::endl
           ...
           << "Usage: create_config_plot -f/--file <input OKS file> -r/--root-object <object UID for session, segment "
              "or application> (-s/--session <session containing root-object>) (-o/--output <output DOT file, default "
              "is config.dot>)"
           << std::endl
           << std::endl
