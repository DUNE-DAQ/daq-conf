...
main(int argc, char* argv[])
{
  ...
  bpo::options_description options_description("Allowed options", 128);
  ...
  options_description.add_options()
    ...
    ("help,h", "Provide help message")
      ...
      ("file,f", bpo::value<std::string>(&oksfilename), "OKS database file name")
        ...
        ("root-object,r",
         bpo::value<std::string>(&object_uid)->default_value(""),
         "OKS object UID of root vertex; must be system, segment or application")(
          "system,s",
          bpo::value<std::string>(&systemname)->default_value(""),
          "Name of the system associated with the root object (only needed if >1 system in the database)")(
          "output,o",
          bpo::value<std::string>(&outputfilename)->default_value("config.dot"),
  ...
  auto display_help_message = [&options_description]() {
    TLOG() << "create_config_plot : Generate dot graphs from database files" << std::endl
           << std::endl
           << "Usage: create_config_plot -f/--file <input OKS file> -r/--root-object <object UID for system, segment "
              "or application> (-s/--system <system containing root-object>) (-o/--output <output DOT file, default is "
              "config.dot>)"
           << std::endl
           << std::endl
  ...
  try {
    bpo::store(bpo::command_line_parser(argc, argv).options(options_description).run(), args);
    bpo::notify(args);
