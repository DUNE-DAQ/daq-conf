...
GraphBuilder::GraphBuilder(const std::string& oksfilename, const std::string& systemname)
  ...
  , m_confdb{ nullptr }
  , m_included_classes{ { ObjectKind::kSystem, { "System", "Segment", "Application" } },
  ...
  , m_system_name{ systemname }
{
  ...
  if (m_system_name == "") { // If no system name given, use the one-and-only system expected in the database
    if (system_objects.size() == 1) {
      ...
      errmsg << "No System instance name was provided, and since " << system_objects.size()
             << " system instances were found in \"" << m_oksfilename << "\" this is an error";
    ...
    if (it == system_objects.end()) {
      std::stringstream errmsg;
      errmsg << "Did not find System instance \"" << m_system_name << "\" in \"" << m_oksfilename
             << "\" and its includes";
      throw daqconf::GeneralGraphToolError(ERS_HERE, errmsg.str());
    }
  }
...
GraphBuilder::calculate_graph(const std::string& root_obj_uid)
{
  ...
  auto it_system =
    std::ranges::find_if(m_all_objects, [&](const ConfigObject& obj) { return obj.UID() == m_system_name; });
...
GraphBuilder::write_graph(const std::string& outputfilename) const
{
  ...
  const std::unordered_map<ObjectKind, VertexStyle> vertex_styles{ { ObjectKind::kSystem, { "octagon", "black" } },
